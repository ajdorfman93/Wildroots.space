...
$(document).ready(function () {
  ...
  // Initialize each carousel's degree
  carousels['carouselWinners'] = { currdeg: 0 };
  carousels['carouselPhilosophy'] = { currdeg: 0 };
  carousels['carouselHistory'] = { currdeg: 0 };
  ...
  // Open popup events
  $("#openWinners").on("click", function (e) {
    e.preventDefault();
    closeAllPopups(); // Close other popups
    $("#carouselWinners").addClass("active");
    $('body').addClass('popup-active');
  });

  $("#openPhilosophy").on("click", function (e) {
    e.preventDefault();
    closeAllPopups(); // Close other popups
    $("#carouselPhilosophy").addClass("active");
    $('body').addClass('popup-active');
  });
  ...
  $("#openHistory").on("click", function (e) {
    e.preventDefault();
    closeAllPopups(); // Close other popups
    $("#carouselHistory").addClass("active");
    $('body').addClass('popup-active');
  });

  // Close popup events
  $(".close-btn").on("click", function () {
    var carouselId = $(this).data('close');
    $("#" + carouselId).removeClass("active");
    $('body').removeClass('popup-active');
  });

  // Carousel navigation
  $(".slider--next2").on("click", function (e) {
    e.stopPropagation(); // Prevent the event from bubbling up
    var carouselId = $(this).data('carousel');
    carousels[carouselId].currdeg -= 60;
    rotate(carouselId);
    fadePopup(carouselId);
  });

  $(".slider--prev2").on("click", function (e) {
    e.stopPropagation(); // Prevent the event from bubbling up
    var carouselId = $(this).data('carousel');
    carousels[carouselId].currdeg += 60;
    rotate(carouselId);
  ...
  // Rotate the carousel
  function rotate(carouselId) {
    $("#" + carouselId + " .carousel").css({
      transform: "rotateY(" + carousels[carouselId].currdeg + "deg)"
    });
  }

  // Fade the popup background
  function fadePopup(carouselId) {
  ...
  // Close popup when any scroll link is clicked
  $(document).on('click', 'a[href^="#"]', function (e) {
    var $clickedLink = $(this);

    // If the link is not a popup opener
    if (!$clickedLink.is('#openWinners, #openPhilosophy, #openHistory')) {
      closeAllPopups();
    }
  });
  ...
  $(document).on('click', function (e) {
    ...
    // If clicking outside the popup content and not on navigation buttons or links
    if (
      !$target.closest('.popup.active').length &&
      !$target.is('a, .slider--next2, .slider--prev2')
    ) {
      closeAllPopups();
    }
  });
